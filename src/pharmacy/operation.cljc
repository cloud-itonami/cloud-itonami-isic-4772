(ns pharmacy.operation
  "OperationActor — one dispense/refill/disclosure/dispute operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (PharmacyOrder-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the PharmacyGovernor (:govern) and the
  rollout phase gate (:decide) before anything is dispensed, logged or
  disclosed.
  ...
  decision to a human reviewer (licensed pharmacist). The reviewer resumes
  with `{:approval {:status :approved}}` (or `:rejected`)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [pharmacy.llm :as llm]
            [pharmacy.policy :as policy]
            [pharmacy.phase :as phase]
...
(defn- commit-record [request _context proposal]
  {:effect  (:effect proposal)
   ...
   :path    [(or (:prescription-id request) (:subject request))]})
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:dispute? verdict) :dispensing-dispute
                                          (:interaction-flag? verdict) :interaction-flag
                                          :else :low-confidence))
                        :phase ph
